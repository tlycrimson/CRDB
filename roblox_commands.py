...
async def sc(ctx, user_id: int):
    try:
        async with ctx.typing():  # <-- This line starts a block
            # EVERYTHING INSIDE THIS BLOCK MUST BE INDENTED!
            user_url = f"{ROBLOX_API_URL}{user_id}"
            user_data = requests.get(user_url, timeout=10).json()
            ...
            embed.add_field(name="Groups", value=", ".join(map(str, groups)) if groups else "Not in groups", inline=False)
